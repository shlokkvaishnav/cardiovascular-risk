...
class PredictionRequest(BaseModel):
    ...
    age: float = Field(
        ...,
        ge=_rng("age", 18, 100)["min"],
        le=_rng("age", 18, 100)["max"],
    ...
    height: float = Field(
        ...,
        ge=_rng("height", 120, 220)["min"],
        le=_rng("height", 120, 220)["max"],
        description="Height (cm)",
    )
    weight: float = Field(
        ...,
        ge=_rng("weight", 30, 250)["min"],
        le=_rng("weight", 30, 250)["max"],
        description="Weight (kg)",
    )
    ap_hi: float = Field(
        ...,
        ge=_rng("ap_hi", 70, 240)["min"],
        le=_rng("ap_hi", 70, 240)["max"],
        description="Systolic blood pressure (mm Hg)",
    )
    ap_lo: float = Field(
        ...,
        ge=_rng("ap_lo", 40, 160)["min"],
        le=_rng("ap_lo", 40, 160)["max"],
        description="Diastolic blood pressure (mm Hg)",
    )
    cholesterol: int = Field(
        ...,
        ge=_rng("cholesterol", 1, 3)["min"],
        le=_rng("cholesterol", 1, 3)["max"],
        description="Cholesterol level (1: normal, 2: above normal, 3: well above normal)",
    )
    gluc: int = Field(
        ...,
        ge=_rng("gluc", 1, 3)["min"],
        le=_rng("gluc", 1, 3)["max"],
        description="Glucose level (1: normal, 2: above normal, 3: well above normal)",
    )
    smoke: int = Field(
        ...,
        ge=_rng("smoke", 0, 1)["min"],
        le=_rng("smoke", 0, 1)["max"],
        description="Current smoker (0: no, 1: yes)",
    )
    alco: int = Field(
        ...,
        ge=_rng("alco", 0, 1)["min"],
        le=_rng("alco", 0, 1)["max"],
        description="Alcohol intake (0: no, 1: yes)",
    )
    active: int = Field(
        ...,
        ge=_rng("active", 0, 1)["min"],
        le=_rng("active", 0, 1)["max"],
        description="Physically active (0: no, 1: yes)",
    )
...
class PredictionResponse(BaseModel):
    ...
    probability: float = Field(
        ..., ge=0, le=1, description="Probability of heart disease"
    )
    risk_level: str = Field(..., description="Risk level: Low, Medium, or High")
    confidence: float = Field(..., description="Model confidence score")
    timestamp: str = Field(..., description="Prediction timestamp")
    request_id: Optional[str] = Field(
        None, description="Unique trace ID for the request"
    )
