...
class DataValidator:
    ...
    def _define_validation_rules(self) -> Dict[str, Dict]:
        ...
        rules: Dict[str, Dict] = {
            name: {"type": "numeric", **rng(name)} for name in numerical
        }
        rules.update(
            {name: {"type": "categorical", **rng(name)} for name in categorical}
        )
        rules["target"] = (
            {"type": "categorical", **rng("target")}
            if "target" in ranges
            else {"type": "categorical", "min": 0, "max": 1}
        )
        return rules
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        missing_cols = [
            col for col in required_cols if col not in df.columns and col != "target"
        ]
    ...
    def _validate_statistics(self, df: pd.DataFrame) -> List[str]:
        ...
        for col in numeric_cols:
            ...
            if std > 0:
                ...
                if len(outliers) > 0:
                    logger.warning(
                        f"Column '{col}' has {len(outliers)} extreme outliers"
                    )
    ...
    def validate_single_instance(self, data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for feature, value in data.items():
            ...
            if rules["type"] == "numeric" and not isinstance(value, (int, float)):
                errors.append(
                    f"Feature '{feature}' should be numeric, got {type(value)}"
                )
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, rules in self.validation_rules.items():
            ...
            if "min" in rules and "max" in rules:
                df_clean = df_clean[
                    (df_clean[col] >= rules["min"]) & (df_clean[col] <= rules["max"])
                ]
        ...
        if removed_rows > 0:
            logger.info(
                f"Removed {removed_rows} invalid/duplicate rows during cleaning"
            )
